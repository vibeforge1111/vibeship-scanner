<script lang="ts">
	...
	let status = $state<'loading' | 'need_auth' | 'confirm_auth' | 'authenticating' | 'success' | 'error' | 'expired'>('loading');
	let error = $state('');
	let storedGithubToken = $state<string | null>(null);
	...
	onMount(async () => {
		...
		auth.subscribe(async (state) => {
			...
			if (!state.user) {
				...
				// User is logged in - require explicit confirmation before completing auth
				storedGithubToken = state.githubToken;
				status = 'confirm_auth';
			} else {
	...
	async function confirmAndCompleteAuth() {
		if (!storedGithubToken) return;
		status = 'authenticating';
		await completeAuth(storedGithubToken);
	}
...
<div class="auth-page">
	<div class="auth-container">
		...
		{:else if status === 'need_auth'}
			...
			<p>Sign in with GitHub to enable<br/>private repo scanning in your IDE.</p>
		...
		{:else if status === 'confirm_auth'}
			<h1>Confirm Authentication</h1>
			<p>You are signed in. Do you want to link your account to this device authentication request?</p>
			<p class="warning-text">Only confirm if you initiated this request from your IDE.</p>
			<button class="confirm-btn" onclick={confirmAndCompleteAuth}>
